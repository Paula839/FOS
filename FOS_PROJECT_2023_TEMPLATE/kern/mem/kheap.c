...
void* sbrk(int increment)
{
	...
	if(increment > 0) {
		...
		if(ROUNDUP(brk + new_increment, PAGE_SIZE) > hardlimit)
			panic("HARDLIMIT !!");
		uint32 old_sbrk = brk;
		//1.1.move the segment break of the kernel to increase the size of its heap
		brk = ROUNDUP(brk + new_increment, PAGE_SIZE);
...
//	new_increment = ROUNDDOWN(increment, PAGE_SIZE); 3'lt
	...
	uint32 i=ROUNDDOWN(brk, PAGE_SIZE);
	if(i==brk)
		i-=PAGE_SIZE;

	for( ;i >= brk + increment; i -=  PAGE_SIZE) {
...
void *krealloc(void *virtual_address, uint32 new_size)
{
	...
	if(va >= hstart && va < hardlimit) {
		if(new_size <= DYN_ALLOC_MAX_BLOCK_SIZE)
			return realloc_block_FF((void*)va, new_size);
		else {
	...
	if(va >= hardlimit + PAGE_SIZE && va < KERNEL_HEAP_MAX) {
		if(new_size == 0) {
			kfree((void*)va);
			return NULL;
		}
		...
		if(ptr_frame_info == NULL || ptr_frame_info->va == 0) return NULL;
		//EQUAL DO NOTHING
//		cprintf("ELSIZE BTA3 DA W DA = %u w %u\n", ROUNDUP(ptr_frame_info->size, PAGE_SIZE), ROUNDUP(new_size, PAGE_SIZE));
		...
		for(uint32 i = newVa - PAGE_SIZE; i >= endVa ; i-=PAGE_SIZE){
