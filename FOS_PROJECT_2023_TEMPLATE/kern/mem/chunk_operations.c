...
//			struct FrameInfo* ptr_frame_info = get_frame_info(e->env_page_directory, i, &ptr_page_table);
		...
		if(ptr_frame_info->element &&ROUNDDOWN(ptr_frame_info->element->virtual_address, PAGE_SIZE) == ROUNDDOWN(i,PAGE_SIZE))
		{
		...
		int perm = pt_get_page_permissions(e->env_page_directory, i);
			...
			tlb_invalidate(e->env_page_directory, (void*) virtual_address);
