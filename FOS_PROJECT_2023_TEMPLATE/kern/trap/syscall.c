...
void* sys_sbrk(int increment)
{
    ...
    uint32 new_increment;
    uint32 old_sbrk = env->brk;
    if(increment>0){
    	if(ROUNDUP(env->brk + increment, PAGE_SIZE) > env->hardlimit){
    	    		return (void*)-1;
    	 }
    	if(old_sbrk%PAGE_SIZE==0){
    	new_increment = ROUNDUP(increment, PAGE_SIZE);
    	env->brk+=new_increment;
    	}
    	else{
    		new_increment = ROUNDUP((increment+old_sbrk), PAGE_SIZE);
    		env->brk=new_increment;
    	}
    	allocate_user_mem(env ,ROUNDUP(old_sbrk, PAGE_SIZE), ROUNDUP(increment, PAGE_SIZE));
    	return (void*)old_sbrk;
    }
    if(increment<0) {
		if(increment+old_sbrk>=KERNEL_HEAP_START){
			uint32 x=ROUNDDOWN(env->brk, PAGE_SIZE);
				uint32 i=env->brk;
				if(x==env->brk){
					i-=PAGE_SIZE;
				}
			  uint32* ptr_page_table;
			  struct FrameInfo* ptr_frame_info = get_frame_info(env->env_page_directory,env->brk,&ptr_page_table);

		    env->brk+=increment;
		    free_user_mem(env, ROUNDUP(env->brk, PAGE_SIZE), -increment);
		    return (void*)env->brk;
    }
   return (void*)-1;
